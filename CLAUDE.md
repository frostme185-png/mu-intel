...
9. **Highlight tên riêng trên dashboard dùng heuristic viết hoa + danh sách tự fetch, không dùng NLP/AI.** Đã thử spaCy (`en_core_web_sm`) thật trên data thật trước khi quyết định — model gắn nhầm tên CLB ("Man United") thành PERSON và miss tên cầu thủ ít phổ biến, tệ hơn heuristic. Thay vì blocklist gõ tay (dễ sót), `collect_match_data.py` tự fetch danh sách CLB Premier League (`/teams`) + đội hình MUFC hiện tại (`/players/squads`) + HLV đang tại nhiệm (`/coachs`, chọn theo career stint có `end: null` mới nhất) qua API-Football, lưu vào `report["known_entities"]`. Chỉ còn phần thực sự ổn định lâu dài (giải đấu, media, vài CLB châu Âu lớn, huyền thoại/HLV nổi tiếng ngoài đội hình) giữ tĩnh trong `dashboard/src/App.jsx`. Trade-off đã biết: cầu thủ đang là rumor/mục tiêu chuyển nhượng (chưa ký) sẽ không được nhận diện khi chỉ nhắc 1 mình (không có trong squad thật), nhưng vẫn được bắt bình thường khi nhắc đầy đủ họ tên (rule 2+ từ viết hoa không phụ thuộc danh sách nào).
